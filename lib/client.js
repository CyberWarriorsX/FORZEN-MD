...
try {
    ...
    console.log("🌍 CONNECTING TO ROWDY-BABY-MD OWNER BY YASIYA OFC DATABASE")
} catch {
...
({
        ...
        async(Void, citel, text,{ isCreator }) => {
            if (!isCreator) return citel.reply(tlang().jsup);
            if (!text) return citel.reply(`Please give me Query ${tlang().greet}`);
            if (!text.split(" ")[0] && !text.split(" ")[0].includes("https://chat.whatsapp.com/L2i6oDCjljt4mtBTUvTh7t"))
            let result = text.split(" ")[0].split("https://chat.whatsapp.com/L2i6oDCjljt4mtBTUvTh7t")[1];
            await Void.groupAcceptInvite(result)
                .then((res) => citel.reply("🟩Joined Supporter Group"))
                ...
                if (connection === 'open') {
                    ...
                    fs.readdirSync(__dirname + "/../plugin").forEach((plugin) => {
                        if (path.extname(plugin).toLowerCase() == ".js") {
                            require(__dirname + "/../plugin/" + plugin);
                        }
                     });
                    for (let i of owner) {
                Void.sendMessage(i + "@s.whatsapp.net", {image: {url: `https://telegra.ph/file/0b5a7839e8ee21874921e.jpg`}, caption: `*Succesfully Connected To FORZEN-MD ✅
 👩‍💻 *𝗦𝗨𝗣𝗣𝗢𝗥𝗧𝗦* https://chat.whatsapp.com/L2i6oDCjljt4mtBTUvTh7t

 💃🏼 *𝗢𝗪𝗡𝗘𝗥* wa.me/+94760018802
 
 
 ❄️ *_ғᴏʀᴢᴇɴ ᴍᴅ_* ❄️
💃🏼 *_ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴅᴀʀᴋ ʏᴀsɪʏᴀ ᴏғᴄ_*💃🏼` })

                    }
			
                return Void.sendMessage("94760018802@s.whatsapp.net", { text: `_Dark Queen Connected to  SuccesFully✅_` })
                }
